...
- Computer Use compatibility: when direct MCP tools such as mcp_computer_use_list_apps, mcp_computer_use_get_app_state, mcp_computer_use_click, or mcp_computer_use_type_text are available, use them directly. They are Forge's supported Computer Use transport. Do not use node_repl for Computer Use when these direct MCP tools are present, even if the loaded skill describes a node_repl transport.
