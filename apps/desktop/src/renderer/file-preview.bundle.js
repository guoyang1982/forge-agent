...
`;
  ...
  // src/renderer/file-preview-entry.js
  var LANG_BY_EXT = {
    java: "java",
    js: "javascript",
    mjs: "javascript",
  ...
  marked.setOptions({
    ...
    breaks: false,
    highlight(code, lang) {
      const language = lang && core_default.getLanguage(lang) ? lang : void 0;
      if (language) {
  ...
  function indexSymbols(text2, lang) {
  ...
  function highlightLine(line, lang) {
    if (lang && core_default.getLanguage(lang)) {
      try {
        return core_default.highlight(line, { language: lang, ignoreIllegals: true }).value;
      } catch {
      }
    }
  ...
  function getIdentFromClick(e, contentEl) {
    ...
    if (node.nodeType !== Node.TEXT_NODE) {
      ...
      const before2 = t.slice(0, offset);
      const m = before2.match(/[A-Za-z_$]\w*$/);
      return m?.[0] || "";
    }
    const text2 = node.textContent || "";
    const before = text2.slice(0, offset);
    const after = text2.slice(offset);
  ...
  function renderMarkdown(container, content) {
    ...
    const html3 = purify.sanitize(marked.parse(raw), {
      ADD_TAGS: ["input"],
  ...
  function renderCode(container, content, path) {
    ...
    const { defs, byName } = indexSymbols(text2, lang);
    container.innerHTML = "";
    ...
    if (defs.length) {
      ...
      nav.innerHTML = defs.slice(0, 60).map(
        (d) => `<button type="button" class="code-symbol-chip" data-line="${d.line}" title="\u7B2C ${d.line} \u884C">${escapeHtml(d.kind)} ${escapeHtml(d.name)}</button>`
      ).join("");
    ...
    body.addEventListener("click", (e) => {
      ...
      if (!row) return;
      const content2 = row.querySelector(".code-line-content");
      if (!content2) return;
      const ident = getIdentFromClick(e, content2);
      if (!ident) return;
