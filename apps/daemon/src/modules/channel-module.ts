...
export function createChannelModule(): DaemonModule<ForgeDaemonContext> {
  return {
    ...
    register(router, context) {
      ...
      router.registerLegacy(DAEMON_METHODS.LIST_CHANNELS, async (params) =>
        handleListChannels(params, deps));
      router.registerLegacy(DAEMON_METHODS.GET_CHANNEL, async (params) =>
        handleGetChannel(params, deps));
      router.registerLegacy(DAEMON_METHODS.CREATE_CHANNEL, async (params) =>
        handleCreateChannel(params, deps));
      router.registerLegacy(DAEMON_METHODS.UPDATE_CHANNEL, async (params) =>
        handleUpdateChannel(params, deps));
      router.registerLegacy(DAEMON_METHODS.DELETE_CHANNEL, async (params) =>
        handleDeleteChannel(params, deps));
      router.registerLegacy(DAEMON_METHODS.LIST_CHANNEL_KINDS, async () =>
        handleListChannelKinds());
      router.registerLegacy(DAEMON_METHODS.GET_CHANNEL_GATEWAY_STATUS, async () =>
        handleGetChannelGatewayStatus(deps));
      router.registerLegacy(DAEMON_METHODS.START_CHANNEL_GATEWAY, async (params) =>
        handleStartChannelGateway(params, deps));
      router.registerLegacy(DAEMON_METHODS.STOP_CHANNEL_GATEWAY, async () =>
        handleStopChannelGateway(deps));
      router.registerLegacy(DAEMON_METHODS.CHANNEL_START_LOGIN, async (params) =>
        handleChannelStartLogin(params, deps));
      router.registerLegacy(DAEMON_METHODS.CHANNEL_POLL_LOGIN, async (params) =>
        handleChannelPollLogin(params, deps));
      router.registerLegacy(DAEMON_METHODS.MOBILE_CREATE_PAIRING, async (params) =>
        handleMobileCreatePairing(params, deps));
      router.registerLegacy(DAEMON_METHODS.MOBILE_LIST_DEVICES, async (params) =>
        handleMobileListDevices(params, deps));
      router.registerLegacy(DAEMON_METHODS.MOBILE_REVOKE_DEVICE, async (params) =>
        handleMobileRevokeDevice(params, deps));
      router.registerLegacy(DAEMON_METHODS.MOBILE_UPDATE_DEVICE_PROJECTS, async (params) =>
...
function registerMobileWorkspaceMethods(
  ...
  router.registerLegacy(DAEMON_METHODS.MOBILE_GIT_BRANCHES, async (params) => {
    const payload = mobileWorkspaceParams(params);
    return handleMobileGitBranches({ cwd: payload.cwd });
  });
  router.registerLegacy(DAEMON_METHODS.MOBILE_GIT_SWITCH, async (params) => {
    const payload = mobileWorkspaceParams(params);
  ...
  router.registerLegacy(DAEMON_METHODS.MOBILE_WORKSPACE_FILES_LIST, async (params) => {
    const payload = mobileWorkspaceParams(params);
    return handleMobileFilesList({ cwd: payload.cwd, path: payload.path });
  });
  router.registerLegacy(DAEMON_METHODS.MOBILE_WORKSPACE_FILE_READ, async (params) => {
    const payload = mobileWorkspaceParams(params);
    return handleMobileFileRead({ cwd: payload.cwd, path: payload.path ?? "" });
  });
  router.registerLegacy(DAEMON_METHODS.MOBILE_WORKSPACE_DIFF_LIST, async (params) => {
    const payload = mobileWorkspaceParams(params);
    return handleMobileDiffList({ cwd: payload.cwd });
  });
  router.registerLegacy(DAEMON_METHODS.MOBILE_WORKSPACE_DIFF_GET, async (params) => {
    const payload = mobileWorkspaceParams(params);
